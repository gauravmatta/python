...
# 6) ImportError: cannot import name 'runtime_version' from 'google.protobuf'
#    (often after TensorFlow + transformers pull in protobuf; your protobuf wheel may be incomplete):
#    → pip install --upgrade "protobuf>=5.28,<6"
#    TensorFlow 2.20+ needs protobuf 5.x. If Streamlit complains about protobuf<5, upgrade Streamlit:
#    → pip install --upgrade "streamlit>=1.40"
# 7) RuntimeError: Failed to find C compiler. Please specify via CC environment variable
#    Reinstalling PyTorch does NOT fix this — PyTorch wheels are prebuilt; the error comes from
#    Unsloth’s stack (e.g. Triton / JIT kernels) trying to compile native code on your machine.
#    On Windows: install “Build Tools for Visual Studio 2022” → workload “Desktop development
#    with C++” (MSVC + Windows SDK), then open a new terminal and run 37.py again.
#    Alternative: use WSL2 (Linux) for Unsloth — fewer compiler issues than native Windows.
#    See: https://docs.unsloth.ai/get-started/installing-+-updating/windows-installation
# 8) ModuleNotFoundError: No module named 'triton' (common on Windows)
#    The PyPI package is named triton on Linux; on Windows install the Windows build:
#    → pip install triton-windows
#    If import still fails, use Python 3.10 or 3.11 from python.org (not 3.13+) in a fresh venv —
#    many ML wheels are tested on 3.10/3.11 first. Official: Unsloth Windows install page above.
# 9) Lost a lot of disk space after reinstalling packages many times
#    → pip cache purge   (clears downloaded .whl files pip kept for reuse — often several GB)
#    → Hugging Face cache: large models live under %USERPROFILE%\.cache\huggingface — remove old
#      model folders you no longer need, or set HF_HOME to another drive if space is tight.
...
def main():
    ...
    # messages: list of\ndicts with "role" ("user" / "assistant" / "system") and "content" (str). One user message = one turn.
